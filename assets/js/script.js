...
function renderNotes(filteredNotes) {
  ...
  if (notesToRender.length === 0) {
    ...
    container.innerHTML = `
      <div class="empty-state">
        ...
        <p>${currentFilter === "archived" ? "<h1>No archived notes found.</h1><br> Create a new note, or archive some existing notes!" : "<h1>No notes found.</h1><br> Create your first note to get started!"}</p>
  ...
  container.innerHTML = sortedNotes
    ...
    <div class="note-card">
      <div class="note-body">
        <h3 class="note-title">
          <i class="pin-icon ri-${note.pinned ? "pushpin-fill" : "pushpin-line"}" onclick="togglePin('${note.id}')" title="${note.pinned ? "Unpin Note" : "Pin Note"
    ...
    try {
      ...
      importedNotes.forEach((note) => {
        ...
        if (
          ...
          // Check if the note already exists to avoid duplicates based on ID
...
function changeColorTheme(colorName) {
  ...
  localStorage.setItem("colorTheme", colorName); // Save the selected color theme
...
function applyStoredColorTheme() {
  ...
  if (storedColorTheme) {
    ...
    changeColorTheme("blue"); // Default color theme
  }
}
...
function handleResize() {
  ...
  if (window.innerWidth <= 550) {
    // Mobile view: Ensure sidebar is hidden and header is primary navigation
    sidebar.classList.add("collapsed"); // Collapse sidebar to hide its content on mobile
    sidebar.style.display = "none"; // Completely hide the sidebar element on mobile
    mainWrapper.classList.add("sidebar-collapsed"); // Adjust main content area for hidden sidebar
    headerBar.classList.remove("sidebar-collapsed"); // Header takes full width on mobile
    // The mobile header expansion is handled by toggleSidebar, so we don't reset is-expanded here
    if (sidebarToggleBtn) {
      sidebarToggleBtn.querySelector("i").className = "ri-menu-line"; // Ensure menu icon on mobile
    }
    if (headerExpandedContent) {
      headerExpandedContent.style.display = 'none'; // Header expanded content should be hidden initially on mobile
    }
    if (overlay) {
      overlay.remove(); // Remove overlay if present on mobile resize
    }
  } else {
    // Desktop/Tablet view: Ensure sidebar is visible and not collapsed initially
    sidebar.classList.remove("collapsed"); // Ensure sidebar is not collapsed, showing its content
    sidebar.style.display = ""; // Remove inline style to let CSS handle display (should be 'flex')
    mainWrapper.classList.remove("sidebar-collapsed"); // Restore main content area for visible sidebar
    headerBar.classList.remove("sidebar-collapsed"); // Restore header position for visible sidebar
    headerBar.classList.remove("is-expanded"); // Ensure header is not expanded if coming from mobile
    if (sidebarToggleBtn) {
      sidebarToggleBtn.querySelector("i").className = "ri-menu-line"; // Ensure menu icon on desktop
    }
    if (headerExpandedContent) {
      headerExpandedContent.style.display = 'none'; // Header expanded content should always be hidden on desktop
    }
    if (overlay) {
      overlay.remove(); // Remove overlay if present on desktop resize
    }
  }
}
...
function toggleSidebar() {
  ...
  if (window.innerWidth <= 550) {
    // Mobile view: Toggle header expansion
    ...
    // When manually toggling sidebar collapse on desktop, ensure header is NOT expanded
    headerBar.classList.remove("is-expanded");
    if (sidebarToggleBtn) {
      sidebarToggleBtn.querySelector("i").className = "ri-menu-line"; // Reset icon
    }
    const overlay = document.getElementById("headerOverlay");
    if (overlay) {
      overlay.remove();
    }
    // Ensure headerExpandedContent is hidden on desktop
    if (headerExpandedContent) {
      headerExpandedContent.style.display = 'none';
    }
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("importNotesBtn").addEventListener("click", () => document.getElementById("importNotesInput").click());
  ...
  // Event listeners for sidebar buttons (desktop/tablet)
  document.getElementById("sidebarExportNotesBtn").addEventListener("click", exportNotes);
  document.getElementById("sidebarImportNotesBtn").addEventListener("click", () => document.getElementById("sidebarImportNotesInput").click());
